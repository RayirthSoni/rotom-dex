...
const EXAMPLES: [string, string][] = [
  ['Find a Pokémon', 'Where can I catch Ralts in Emerald?'],
  ['Find an item', 'Where can I get an Exp. Share in Diamond?'],
  ['Understand a mechanic', 'What is STAB?'],
  ['Build a team', 'Help me build a story team around Charmander in Red.'],
]
...
function Answer({exchange, apply}: {exchange: Exchange; apply: (action: ChatAction) => string}) {
  ...
  return <div className="rotom-response" data-testid="chat-answer" data-abstained={a.abstained}>
    <div className="answer-byline"><span className="rotom-mark" aria-hidden="true">R</span><strong>Rotom</strong><span>{a.games?.map(title).join(' · ') || (exchange.game ? title(exchange.game) : 'Pokémon explained')}</span></div>
    ...
    {a.cards?.length > 0 && <div className="answer-cards">{a.cards.map((card,i) => <section className="answer-card" key={i}>
      {card.sprite_url?.startsWith('https://raw.githubusercontent.com/PokeAPI/sprites/') && <img className="answer-sprite" src={card.sprite_url} alt="" loading="lazy"/>}<h3>{title(card.title)}</h3><dl>{card.rows.map((row,j) => <div key={j}><dt>{title(row.label)}</dt><dd>{row.value}</dd></div>)}</dl>
    </section>)}</div>}
    ...
    {a.actions?.map((action,i) => <button className="btn" key={i} disabled={Boolean(applied[i])} onClick={() => setApplied(s => ({...s,[i]:apply(action)}))}>{applied[i] || action.label}</button>)}
  ...
  return <div className="chat-app">
    ...
    {keyOpen && <section className="setup-panel" aria-label="Gemini connection"><h2>Your key. Your conversations.</h2><p>Connect your own Gemini API key. Chat and web research use your quota. The key passes through this server to Google and stays in memory only until you reload or disconnect.</p>
      {!credential.key ? <form onSubmit={async e => {e.preventDefault(); setConnecting(true); setKeyStatus('Testing connection…'); try {await connectKey(keyDraft.trim()); credential.set(keyDraft.trim(),true); setKeyDraft(''); setKeyStatus('Connected. You can ask your question now.'); setKeyOpen(false)} catch(error) {setKeyStatus(error instanceof Error ? error.message : 'Connection failed')} finally {setConnecting(false)}}}>
        <label htmlFor="gemini-key">Gemini API key</label><div className="key-input-row"><input id="gemini-key" type="password" autoComplete="off" value={keyDraft} onChange={e => setKeyDraft(e.target.value)} placeholder="Paste your Gemini key"/><button className="btn" disabled={connecting || !keyDraft.trim()}>{connecting ? 'Connecting…' : 'Connect'}</button></div>
    ...
    <div className="chat-layout">
      ...
      <div className="chat-column">
        ...
        {!active?.exchanges.length && <section className="chat-welcome"><div className="welcome-eyebrow">YOUR POKÉMON COMPANION</div><h1>A little guidance.<br/><span>A better adventure.</span></h1><p>Find your next teammate, track down an item, or make sense of a tricky mechanic. Just ask Rotom.</p><div className="example-grid">{EXAMPLES.map(([label,text]) => <button key={label} onClick={() => {setQuestion(text); input.current?.focus()}}><strong>{label}</strong><span>{text}</span><span aria-hidden="true">↗</span></button>)}</div></section>}
