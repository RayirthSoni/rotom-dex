...
export function CompetitiveScreen() {
  ...
  return <div className="competitive-page"><h1>A team with a plan.</h1><p>Check a team against an exact format, then explore damage ranges. These tools work without a Gemini key.</p>
    ...
    <section className="setup-panel"><h2>2. Bring your team</h2><p>Paste a Pokémon Showdown team. Include abilities, moves, items, natures, EVs, IVs and battle modifiers when relevant. Saved in this browser.</p>
      ...
      <div className="flex gap-2 flex-wrap"><button className="btn btn-primary" disabled={busy || !format || !team.trim()} onClick={() => run(async () => setValidation(await request<Validation>('validate',{format,team})))}>Check legality</button>
