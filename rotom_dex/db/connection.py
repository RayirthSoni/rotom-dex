...
def initialize(db: sqlite3.Connection) -> None:
    ...
    elif "schema_version" not in tables or [
        r[0] for r in db.execute("SELECT version FROM schema_version")
    ] != [1]:
        raise ValueError(
            "Unsupported database schema; create a fresh snapshot database"
        )
