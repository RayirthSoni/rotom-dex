...
from src.ingestion.emerald import DEFAULT_CACHE, DEFAULT_PACK, historical, import_emerald, table_counts, validate_database
...
class FoundationTests(unittest.TestCase):
    ...
    def test_adversarial_other_game_records_do_not_leak(self):
        ...
        self.db.execute('INSERT INTO pokemon_game_data VALUES (280,7,?,?)', ('unknown', evidence))
        self.db.execute("INSERT INTO move_game_data SELECT move_id,7,type_id,damage_class,99,accuracy,pp,effect,evidence_id FROM move_game_data WHERE move_id=93 AND game_id=9")
        self.db.execute("INSERT INTO learnsets VALUES (280,7,93,'level-up',99,NULL,?)", (evidence,))
        self.db.execute("INSERT INTO acquisitions VALUES ('test:ruby',7,280,NULL,NULL,'test-only',1,1,NULL,'unavailable',?,?,?)", ('{"op":"always"}', '{"op":"always"}', evidence))
        ...
        self.assertFalse(any(r['method'] == 'test-only' for r in emerald['acquisition']))
        self.assertEqual(ruby['evolution'], [])
        self.assertTrue(emerald['evolution'])

    def test_historical_types_and_abilities(self):
        self.assertEqual([r['type'] for r in self.query()['data']['types']], ['psychic'])
        self.assertEqual([r['ability'] for r in self.query('breloom')['data']['abilities']], ['effect-spore'])
        self.assertEqual([r['ability'] for r in self.query('zigzagoon')['data']['abilities']], ['pickup'])
        self.assertEqual(self.db.execute('SELECT count(*) FROM pokemon_abilities WHERE is_hidden=1').fetchone()[0], 0)
        self.assertEqual(self.db.execute("SELECT count(*) FROM types WHERE slug='fairy'").fetchone()[0], 0)

    def test_historical_move_values_and_type_based_category(self):
        r = self.db.execute('SELECT power,accuracy,damage_class FROM move_game_data WHERE move_id=33 AND game_id=9').fetchone()
        self.assertEqual(tuple(r), (35, 95, 'physical'))
        r = self.db.execute('SELECT power,damage_class FROM move_game_data WHERE move_id=348 AND game_id=9').fetchone()
        self.assertEqual(tuple(r), (70, 'special'))
        r = self.db.execute('SELECT power,pp FROM move_game_data WHERE move_id=202 AND game_id=9').fetchone()
        self.assertEqual(tuple(r), (60, 5))

    def test_historical_stat_boundaries(self):
        current = [{'stat_id': '2', 'base_stat': '100'}, {'stat_id': '3', 'base_stat': '50'}]
        past = [{'stat_id': '2', 'base_stat': '80', 'generation_id': '5'},
                {'stat_id': '2', 'base_stat': '70', 'generation_id': '3'},
                {'stat_id': '2', 'base_stat': '60', 'generation_id': '1'}]
        self.assertEqual(historical(current, past, 3, ('stat_id',))[0]['base_stat'], '70')
        self.assertEqual(historical(current, past, 4, ('stat_id',))[0]['base_stat'], '80')
        self.assertEqual(historical(current, past, 6, ('stat_id',))[0]['base_stat'], '100')

    def test_encounter_and_evolution_facts(self):
        acquisition = self.query()['data']['acquisition']
        self.assertEqual(len(acquisition), 1)
        self.assertEqual((acquisition[0]['location'], acquisition[0]['min_level'], acquisition[0]['chance_percent']), ('hoenn-route-102', 4, 4))
        self.assertEqual(acquisition[0]['availability'], 'unknown')
        rule = self.query()['data']['evolution'][0]
        self.assertEqual((rule['to_pokemon'], rule['conditions']), ('kirlia', {'op': 'level_at_least', 'value': 20}))
    ...
    def test_missing_encounters_do_not_mean_unavailable(self):
        ...
        self.assertEqual({r['method'] for r in result['data']['acquisition']}, {'evolution'})
        self.assertTrue(all(r['availability'] == 'unknown' for r in result['data']['acquisition']))
        self.assertEqual(next(c['status'] for c in result['coverage'] if c['feature'] == 'acquisition'), 'partial')
        self.assertEqual(self.db.execute("SELECT count(*) FROM acquisitions WHERE availability='unavailable'").fetchone()[0], 0)

    def test_machine_relationships_and_level_filter(self):
        result = self.query(level=5)['data']['learnset']
        self.assertEqual([r['move'] for r in result if r['method'] == 'level-up'], ['growl'])
        machines = [r for r in result if r['method'] == 'machine']
        self.assertTrue(machines)
        self.assertTrue(all(r['machine_item'] and r['machine_acquisition_coverage'] == 'missing' for r in machines))
        with self.assertRaises(ValueError):
            self.query(level=101)

    def test_item_effects_and_sourced_acquisition(self):
        r = self.db.execute('SELECT effect FROM item_game_data WHERE game_id=9 AND item_id=135').fetchone()
        self.assertIn('30 HP', r[0])  # Gen III Sitrus effect, not modern quarter-max-HP.
        acquisitions = self.db.execute('SELECT * FROM acquisitions WHERE game_id=9 AND item_id=132').fetchall()
        ...
        self.assertEqual(self.db.execute("SELECT status FROM coverage WHERE subject='item:17' AND feature='acquisition'").fetchone()[0], 'missing')

    def test_type_chart_and_natures(self):
        self.assertEqual(self.db.execute('SELECT count(*) FROM type_effectiveness').fetchone()[0], 289)
        for attack in (8, 17):
            self.assertEqual(self.db.execute('SELECT damage_factor FROM type_effectiveness WHERE game_id=9 AND attack_type_id=? AND defense_type_id=9', (attack,)).fetchone()[0], 50)
        self.assertEqual(self.db.execute('SELECT damage_factor FROM type_effectiveness WHERE game_id=9 AND attack_type_id=1 AND defense_type_id=8').fetchone()[0], 0)
        self.assertEqual(self.db.execute('SELECT count(*) FROM natures').fetchone()[0], 25)
        self.assertEqual(self.db.execute('SELECT count(*) FROM natures WHERE increased_stat=decreased_stat').fetchone()[0], 5)

    def test_foreign_keys_and_constraints(self):
        evidence = self.db.execute('SELECT id FROM evidence LIMIT 1').fetchone()[0]
        with self.assertRaises(sqlite3.IntegrityError):
            self.db.execute('INSERT INTO pokemon_types VALUES (280,7,1,14,?)', (evidence,))
        with self.assertRaises(sqlite3.IntegrityError):
            self.db.execute('UPDATE evolution_rules SET to_form_id=999999 WHERE id=148')
        with self.assertRaises(sqlite3.IntegrityError):
            self.db.execute("UPDATE acquisitions SET item_id=17 WHERE id='encounter:29603'")
        with self.assertRaises(sqlite3.IntegrityError):
    ...
    def test_conflicting_fact_rolls_back(self):
        self.db.execute("UPDATE pokemon_stats SET base_stat=99 WHERE form_id=280 AND stat='hp'")
        self.db.commit()
        before = '\n'.join(self.db.iterdump())
    ...
    def test_cli_commands_and_errors(self):
        for args in [('check',), ('coverage',), ('query', 'ralts', '--game', 'emerald')]:
        ...
        with contextlib.redirect_stderr(io.StringIO()):
            self.assertEqual(main(['query', 'ralts', '--game', 'emerald', '--db', str(absent)]), 1)
...
class ConditionsTests(unittest.TestCase):
    def test_three_valued_and_or(self):
        ...
        self.assertIsNone(evaluate_condition({'op': 'and', 'args': [level, unknown]}, {'level': 20}))
        self.assertFalse(evaluate_condition({'op': 'and', 'args': [level, unknown]}, {'level': 19}))
        self.assertTrue(evaluate_condition({'op': 'or', 'args': [level, unknown]}, {'level': 20}))
        self.assertIsNone(evaluate_condition(level, {}))

    def test_invalid_conditions_fail_closed(self):
        for value in ({'op': 'and', 'args': []}, {'op': 'arbitrary-script'}, {'op': 'level_at_least', 'value': True}, {'op': 'always', 'ignored': 'x'}):
            with self.assertRaises(ValueError):
