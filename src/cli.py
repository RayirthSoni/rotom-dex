...
def main(argv=None):
    ...
    for name in ('import', 'query', 'coverage', 'check'):
        ...
        if name == 'query':
            command.add_argument('pokemon', help='Slug or national Pokédex number in the sample')
            command.add_argument('--game', required=True, help='Exact game, e.g. emerald')
        ...
        if args.command == 'import':
            ...
            try:
                if args.command == 'query':
                    ...
                    result = rows(db, 'SELECT g.slug AS game,c.* FROM coverage c JOIN game_versions g ON g.id=c.game_id ORDER BY game,subject,feature')
                else:
