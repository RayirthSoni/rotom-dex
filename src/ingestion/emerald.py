...
class Writer:
    ...
    def row(self, table, values):
        ...
        columns = list(values)
        params = [
            canonical(v) if isinstance(v, (dict, list)) else v for v in values.values()
        ]
        self.db.execute(
            f"INSERT INTO {table} ({','.join(columns)}) VALUES ({','.join('?' for _ in params)}) ON CONFLICT DO NOTHING",
            params,
        )
...
def historical(current, past, generation, keys):
    ...
    result = (
        {tuple(r[k] for k in keys): [r] for r in current}
        if keys
        else {(): list(current)}
    )
...
def import_emerald(path, cache_path=DEFAULT_CACHE, pack_path=DEFAULT_PACK):
    ...
    if (
        pack["format_version"],
        pack["game_id"],
        pack["version_group_id"],
        pack["generation"],
    ) != (1, 9, 6, 3):
        raise ValueError("This importer only supports Emerald pack format 1")
    snapshot = hashlib.sha256(
        canonical(cache.manifest).encode() + pack_bytes + b"emerald-normalizer-v1"
    ).hexdigest()
    db = connect(path)
    try:
        initialize(db)
        with db:
            ...
            if existing and [r[0] for r in existing] != [snapshot]:
                raise ValueError(
                    "Different source snapshot; import into a new database and review the changes"
                )
            w = Writer(db)
            w.row(
                "snapshots",
                {"id": snapshot, "pack_sha256": hashlib.sha256(pack_bytes).hexdigest()},
            )
            ...
            w.add(
                m.SourceReference(
                    "emerald-pack",
                    "local:data/game-packs/emerald/pack.json",
                    pack["reviewed_at"],
                    hashlib.sha256(pack_bytes).hexdigest(),
                    snapshot,
                    "data/game-packs/emerald/pack.json",
...
def populate(c, pack, w):
    ...
    for gid in (
        game,
        7,
    ):  # Ruby is a catalog-only negative control, never populated with Emerald facts.
        r = versions[gid]
        g = groups[int(r["version_group_id"])]
        w.add(
            m.GameVersion(
                gid,
                r["identifier"],
                ...
                g["identifier"],
                int(g["generation_id"]),
                "sample" if gid == game else "unsupported",
                ev(("versions", f"id={gid}"), ("version_groups", f"id={g['id']}")),
            )
        )
    w.add(
        m.Coverage(
            ...
            "all",
            "missing",
            "Ruby catalog identity only; no Ruby facts imported.",
            ev(policy),
        )
    )
    species, pokemon, forms = (
        c.index("pokemon_species"),
        c.index("pokemon"),
        c.index("pokemon_forms"),
    )
    ...
    obtainable = {
        ...
        if int(r["version_id"]) == game
    }
    reviewed_rules = [
        r
        for r in c.rows("pokemon_evolution")
        if int(r["id"]) in pack["reviewed_evolution_ids"]
    ]
    for _ in roster:
    ...
    for pid in sorted(roster):
        ...
        if (
            int(p["species_id"]) != pid
            or int(f["pokemon_id"]) != pid
            or f["is_default"] != "1"
            or int(s["generation_id"]) > gen
        ):
            raise ValueError(
                "Roster must contain default forms from generation 3 or earlier"
            )
        ...
        w.add(
            m.PokemonGameData(
                pid,
                game,
                "available" if pid in obtainable else "unknown",
                ev(
                    (
                        "encounters",
                        f"version_id=9; pokemon_id={pid}; or reviewed evolution ancestry",
                    ),
                    ("pokemon_evolution", "reviewed_evolution_ids in emerald-pack"),
                    policy,
                ),
            )
        )
    for name, keys in [
        ...
        for pid in sorted(roster):
            evidence = ev(
                ...
                (name + "_past", f"pokemon_id={pid}; nearest generation_id >= 3"),
                policy,
            )
            for r in historical(now[pid], past[pid], gen, keys):
                if name == "pokemon_types":
                    w.add(
                        m.PokemonType(
                            pid, game, int(r["slot"]), int(r["type_id"]), evidence
                        )
                    )
                    ...
                    if int(r["stat_id"]) <= 6:
                        w.add(
                            m.PokemonStat(
                                pid,
                                game,
                                stats[int(r["stat_id"])]["identifier"],
                                int(r["base_stat"]),
                                evidence,
                            )
                        )
                    ...
                    if (
                        int(abilities[aid]["generation_id"]) > gen
                        or r["is_hidden"] == "1"
                    ):
                        raise ValueError(
                            "Post-Emerald ability survived historical resolution"
                        )
                    w.add(
                        m.Ability(
                            aid,
                            abilities[aid]["identifier"],
                            ev(("abilities", f"id={aid}")),
                        )
                    )
                    w.add(
                        m.PokemonAbility(
                            pid, game, int(r["slot"]), aid, False, evidence
                        )
                    )
    ...
    learnsets = [
        ...
        if int(r["pokemon_id"]) in roster and int(r["version_group_id"]) == group
    ]
    move_ids = {int(r["move_id"]) for r in learnsets}
    machines = {
        int(r["move_id"]): int(r["item_id"])
        for r in c.rows("machines")
        if int(r["version_group_id"]) == group
    }
    held_items = [
        r
        for r in c.rows("pokemon_items")
        if int(r["pokemon_id"]) in roster and int(r["version_id"]) == game
    ]
    item_ids = set(pack["extra_item_ids"]) | {int(r["item_id"]) for r in held_items}
    item_ids |= {
        machines[int(r["move_id"])]
        for r in learnsets
        if r["pokemon_move_method_id"] == "4"
    }
    items = c.index("items")
    flavor = {
        int(r["item_id"]): r["flavor_text"]
        for r in c.rows("item_flavor_text")
        if int(r["version_group_id"]) == group and r["language_id"] == "9"
    }
    for iid in sorted(item_ids):
        ...
        w.add(
            m.ItemGameData(
                ...
                ev(
                    (
                        "item_flavor_text",
                        f"item_id={iid}; version_group_id=6; language_id=9",
                    )
                ),
            )
        )
        w.add(
            m.Coverage(
                ...
                f"item:{iid}",
                "effects",
                "partial" if text else "missing",
                "Emerald in-game description only; not a complete mechanical effect model.",
                ev(policy),
            )
        )
        w.add(
            m.Coverage(
                ...
                "acquisition",
                "partial"
                if any(int(r["item_id"]) == iid for r in held_items)
                else "missing",
                "Only wild held-item routes are derived; shops, field items and gifts are not covered.",
                ev(policy),
            )
        )
    moves = c.index("moves")
    ...
    for r in c.rows("move_changelog"):
        if groups[int(r["changed_in_version_group_id"])]["order"] and int(
            groups[int(r["changed_in_version_group_id"])]["order"]
        ) > int(groups[group]["order"]):
            changes[int(r["move_id"])].append(r)
    for mid in sorted(move_ids):
        r = dict(moves[mid])
        for change in sorted(
            changes[mid],
            key=lambda x: int(groups[int(x["changed_in_version_group_id"])]["order"]),
            reverse=True,
        ):
        ...
        category = (
            "status"
            if r["damage_class_id"] == "1"
            else {"2": "physical", "3": "special"}[types[tid]["damage_class_id"]]
        )
        ...
        w.add(
            m.MoveGameData(
                ...
                tid,
                category,
                int(r["power"]) if r["power"] else None,
                int(r["accuracy"]) if r["accuracy"] else None,
                int(r["pp"]),
                None,
                ev(
                    ("moves", f"id={mid}"),
                    (
                        "move_changelog",
                        f"move_id={mid}; rewind changes after version_group_id=6",
                    ),
                    ("types", f"id={tid}; generation-3 damage class"),
                    policy,
                ),
            )
        )
    ...
    for r in learnsets:
        ...
        refs = [
            (
                "pokemon_moves",
                f"pokemon_id={pid}; version_group_id=6; move_id={mid}; method_id={r['pokemon_move_method_id']}; level={r['level']}",
            )
        ]
        if method == "machine":
        ...
        w.add(
            m.LearnsetEntry(
                ...
                method,
                int(r["level"]),
                machines[mid] if method == "machine" else None,
                ev(*refs),
            )
        )
    ...
    for rid in pack["reviewed_evolution_ids"]:
        ...
        origin = int(species[target]["evolves_from_species_id"])
        ...
        if (
            set(meaningful) != expected
            or r["evolution_trigger_id"] != "1"
            or r["version_group_id"] != "5"
        ):
        ...
        w.add(
            m.EvolutionRule(rid, game, origin, target, "level-up", condition, evidence)
        )
        w.add(
            m.Acquisition(
                f"evolution:{game}:{rid}",
                game,
                target,
                None,
                None,
                ...
                "unknown",
                {
                    "op": "and",
                    "args": [
                        {"op": "has_pokemon", "value": pokemon[origin]["identifier"]},
                        condition,
                    ],
                },
                {"op": "always"},
    ...
    encounters = [
        r
        for r in c.rows("encounters")
        if int(r["version_id"]) == game and int(r["pokemon_id"]) in roster
    ]
    slots, areas, locations = (
        c.index("encounter_slots"),
        c.index("location_areas"),
        c.index("locations"),
    )
    ...
    for r in c.rows("encounter_condition_value_map"):
        condition_map[int(r["encounter_id"])].append(
            conditions[int(r["encounter_condition_value_id"])]["identifier"]
        )
    for r in encounters:
        ...
        w.add(
            m.Location(
                ...
                area["identifier"],
                ev(
                    ("location_areas", f"id={lid}"),
                    ("locations", f"id={area['location_id']}"),
                ),
            )
        )
        prerequisites = {
            ...
            "args": [
                ...
                unknown(
                    "Progression, method requirements and gift restrictions have not been reviewed."
                ),
            ],
        }
        encounter_condition = (
            {
                "op": "and",
                "args": [
                    {"op": "encounter_condition", "value": value}
                    for value in condition_map[eid]
                ],
            }
            if condition_map[eid]
            else {"op": "always"}
        )
        evidence = ev(
            ("encounters", f"id={eid}; version_id=9"),
            ("encounter_slots", f"id={r['encounter_slot_id']}"),
                    ...
                    m.Acquisition(
                        ...
                        "unknown",
                        {
                            ...
                            "args": [
                                prerequisites,
                                {
                                    "op": "encounter_pokemon",
                                    "value": pokemon[pid]["identifier"],
                                },
                            ],
                        },
                        encounter_condition,
                        ev(
                            (
                                "encounters",
                                f"id={eid}; wild {pokemon[pid]['identifier']}",
                            ),
                            (
                                "pokemon_items",
                                f"pokemon_id={pid}; version_id=9; item_id={iid}",
                            ),
                            policy,
                        ),
                    )
    ...
    for r in c.rows("natures"):
        w.add(
            m.Nature(
                ...
                game,
                r["identifier"],
                stats[int(r["increased_stat_id"])]["identifier"],
                stats[int(r["decreased_stat_id"])]["identifier"],
                ev(("natures", f"id={r['id']}"), policy),
            )
        )
    ...
    for r in c.rows("type_efficacy"):
        ...
        if a not in types or d not in types:
            continue
        applicable = [
            p for p in past_efficacy[(str(a), str(d))] if int(p["generation_id"]) >= gen
        ]
        value = (
            min(applicable, key=lambda p: int(p["generation_id"])) if applicable else r
        )
        w.add(
            m.TypeEffectiveness(
                game,
                a,
                d,
                int(value["damage_factor"]),
                ev(
                    ("type_efficacy", f"damage_type_id={a}; target_type_id={d}"),
                    (
                        "type_efficacy_past",
                        f"damage_type_id={a}; target_type_id={d}; nearest generation_id >= 3",
                    ),
                    policy,
                ),
            )
        ...
        for feature in ("types", "stats", "abilities", "learnset", "evolution"):
            w.add(
                m.Coverage(
                    ...
                    feature,
                    "complete",
                    "Complete for this default form within the pinned source and reviewed Emerald mapping; not independently game-tested.",
                    ev(policy),
                )
            )
        w.add(
            m.Coverage(
                ...
                "acquisition",
                "partial",
                "Imported exact-version encounters and reviewed evolution routes. Breeding/trades and progression prerequisites remain unreviewed; absent rows do not mean unavailable.",
                ev(policy),
            )
        )
        w.add(
            m.Coverage(
                game, subject, "prerequisites", "missing", pack["gaps"], ev(policy)
            )
        )
    for feature, status, note in [
        (
            "roster",
            "partial",
            "10 default forms; all other Pokémon outside sample coverage.",
        ),
        (
            "natures",
            "complete",
            "25 natures. Identical increase/decrease stats mean neutral.",
        ),
        (
            "type-effectiveness",
            "complete",
            "17x17 Emerald chart; historical Steel resistances applied.",
        ),
        (
            "move-effects",
            "missing",
            "Historical power/accuracy/PP/type/category included; effect descriptions deliberately NULL.",
        ),
        (
            "ability-effects",
            "missing",
            "Ability slots only; ability effects not imported.",
        ),
        (
            "progression",
            "missing",
            "No reachability graph, milestones or boss data in this foundation.",
        ),
    ]:
...
def validate_database(db):
    ...
    for r in db.execute("SELECT form_id,game_id FROM pokemon_game_data"):
        if (
            db.execute(
                "SELECT count(*) FROM pokemon_stats WHERE form_id=? AND game_id=?",
                tuple(r),
            ).fetchone()[0]
            != 6
        ):
        ...
        for table in ("pokemon_types", "pokemon_abilities", "learnsets"):
            if not db.execute(
                f"SELECT 1 FROM {table} WHERE form_id=? AND game_id=?", tuple(r)
            ).fetchone():
                raise ValueError(f"Missing required {table}")
    if (
        db.execute(
            "SELECT count(*) FROM type_effectiveness WHERE game_id=9"
        ).fetchone()[0]
        != 289
    ):
    ...
    if db.execute(
        "SELECT 1 FROM evidence e LEFT JOIN evidence_members m ON e.id=m.evidence_id WHERE m.evidence_id IS NULL"
    ).fetchone():
